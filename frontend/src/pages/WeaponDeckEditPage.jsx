...
const makeLocalId = () => `weapon-${Date.now()}-${Math.random().toString(16).slice(2, 8)}`;

export default function WeaponDeckEditPage() {
  ...
  useEffect(() => {
    ...
    const loadDeck = async () => {
      try {
        setLoading(true);
        const res = await fetch(`${apiBase}/api/custom/weapon-decks/${deckName}`, { headers: authHeaders });
        ...
        setWeapons((data.weapons || []).map((c) => ({ ...c, tags: parseTags(c), localId: c.localId || makeLocalId() })));
      } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(null);
      const payload = {
        weapons: weapons.map((c) => normalizeCard(c)),
      };
      const res = await fetch(`${apiBase}/api/custom/weapon-decks/${deckName}`, {
        method: "POST",
        headers: jsonHeaders,
        body: JSON.stringify(payload),
      });
      if (!res.ok) throw new Error("Failed to save deck");
      navigate("/forge/weapons");
    } catch (e) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-amber-300">Editing Weapon Deck: {deckName}</h1>
          <p className="text-slate-400 text-sm">Edit weapons, then save the deck.</p>
      ...
      <section className="bg-slate-900/70 border border-slate-800 rounded-xl p-3 space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-amber-200">Weapons</h2>
          <span className="text-xs text-slate-400">{weapons.length} cards</span>
        </div>
        <div className="grid md:grid-cols-2 gap-3">
          {weapons.map((w, idx) => (
            <div key={w.localId || idx} className="bg-slate-900 border border-slate-800 rounded-lg p-3 space-y-2">
