...
const ProfilePage = ({ onReturnToLobby }) => {
  const { user, token } = useStore(); // Use token for authenticated requests if needed
    ...
    const fetchProfile = async () => {
      ...
      try {
        const response = await fetch(
          ...
          {
            headers: {
              // If you secure this endpoint, you'll need the token
            ...
            .map((game) => (
              <div
                ...
                className={`p-3 rounded-md flex justify-between items-center ${
                  game.is_win ? "bg-green-800/50" : "bg-red-800/50"
                }`}
              >
                <div>
                ...
                <span
                  className={`font-bold text-lg ${
                    game.is_win ? "text-green-300" : "text-red-300"
                  }`}
                >
