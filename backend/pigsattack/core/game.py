...
class Game:
    ...
    def eliminate_player_by_id(self, player_id: int):
        """Finds a player by their controller's ID and eliminates them."""
        player_to_eliminate = next((p for p in self._players if hasattr(p.controller, 'player_index') and p.controller.player_index == player_id), None)
        
        if player_to_eliminate and not player_to_eliminate.is_eliminated:
            player_to_eliminate.is_eliminated = True
            # Discard their hand
            for card in player_to_eliminate.hand: self._deck.discard(card)
            player_to_eliminate.hand = []
            self._log_and_sync(f"{player_to_eliminate.name} has been eliminated from the game.")
