...
class ServerView(GameView):
    ...
    def __init__(self, client_manager: ClientManager, socketio):
    ...
    def send_to_client(self, client: Client, message: Dict[str, Any]):
        self.socketio.emit('message', message, to=client.sid)
    ...
    def _broadcast(self, room: GameRoom, message: Dict[str, Any]):
        """Broadcasts a message to all clients in a specific Socket.IO room."""
        # We use the room's ID as the Socket.IO room name
        self.socketio.emit('message', message, to=room.room_id)
    ...
    def display_defense_result(self, success: bool, total_defense: int, attack_strength: int, attacker_name: str, cards_played: List[str], room: GameRoom):
        ...
        self._broadcast(room, {"type": "event", "message": message})

    def display_event(self, event_card, event_name, room): self._broadcast(room, {"type": "event", "message": f"EVENT: {event_name} ({event_card})"})
    def display_action_result(self, message: str, room): self._broadcast(room, {"type": "event", "message": f"ACTION: {message}"})
    def announce_winner(self, player, room): self._broadcast(room, {"type": "event", "message": f"GAME OVER! Winner is {player.name if player else 'NO ONE'}!"})
    def display_turn_start(self, player: Player, room): self._broadcast(room, {"type": "event", "message": f"It is now {player.name}'s turn."})
    def display_event_result(self, message: str, room): self._broadcast(room, {"type": "event", "message": f"{message}"})
    def announce_nightfall(self, room): self._broadcast(room, {"type": "event", "message": "NIGHT HAS FALLEN!"})
